...
```{bash}
head -2 ../output/95_blastp.tab
wc -l ../output/95_blastp.tab
```
...
```{bash}
tr '|' '\t' < ../output/95_blastp.tab | head -2
```


```{bash}
tr '|' '\t' < ../output/95_blastp.tab \
> ../output/95_blastp.tab
```
...
```{r}
bltabl <- read.csv("../output/95_blastp.tab", sep = '\t', header = FALSE)
```
