...
blast2.0_file <- blast_file[, c("V1", "V11")]
...
ggplot(blast2.0_file_df, aes(x = V11)) +
  geom_histogram(binwidth = 50, fill = "blue", alpha = 0.5) +
  labs(title = "Histogram of V11", x = "V11", y = "Frequency")

# Plot boxplot using ggplot2

ggplot(blast2.0_file_df, aes(x = V1, y = V11)) +
  geom_boxplot() +
  xlab("Group") +
  ylab("Value") +
  ggtitle("Boxplot of V11 by V1")


# Plot histogram using ggplot2
ggplot(blast2.0_file_df, aes(x = Length)) +
  geom_histogram(binwidth = 1, color = "purple", fill = "red", alpha = 0.75) +
  labs(title = "Histogram of P-Sequence Lengths",
       x = "P-Sequence Length",
       y = "Frequency") +
...
colnames(bltabl) <- c("V1", "V2", "V3","V4", "V5", "V6", "V7", "V8", "V9", "V10","V11", "V12")
```

```{r}
spgo <- read.csv("../output/95protein.tab", sep = '\t', header = TRUE)
colnames(spgo) <- c("V1", "V2", "V3")
```
...
kbl(
head(
  left_join(bltabl, spgo,  by = c("V1" = "Entry")) %>%
  select(V1, V2, V3) %>% mutate(V1 = str_replace_all(V1, 
...
kbl(
  head(
    left_join(bltabl, spgo, by = c("V1" = "Entry")) %>%
      ...
      rename(First_Column = V1, Second_Column = V2, Third_Column = V3)
  )
) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"))
